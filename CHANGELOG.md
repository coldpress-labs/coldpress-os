...
- **Graphify schema reshape to coldpress-os folder semantics** — Graphify ships generic extraction; the `coldpress` namespace extension (`node_type`, `env_tag`, `dir_role`) is populated by indexer adapters that haven't been built yet (Wave 3 Block O). Today, running `coldpress graph rebuild` produces a graph without `coldpress` metadata (hence the `<unclassified>` / `<untagged>` buckets in smoke-test output).
