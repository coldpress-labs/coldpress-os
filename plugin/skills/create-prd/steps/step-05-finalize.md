...
1. **Compile the PRD:**
   ...
   - Include YAML frontmatter with sacred document metadata:
     ...
     created: "{date}"
     last_modified: "{date}"
     governance: "draft"          # author as draft (editable); flipped to "locked" at lock (step 6) AFTER sign-off. Never stamp requires-review/locked here — it freezes the PRD mid-authoring (VP2 O16).
     workflowType: "prd"
...
6. **Write to disk as draft, then lock only after sign-off:**
   ...
   - **Only after the user's final sign-off (step 5)**, flip frontmatter `governance: "draft"` → `governance: "locked"`. This flip is the lock `sacred-guard` enforces; post-lock edits require the `sacred-change` workflow. Locking before sign-off freezes the PRD mid-authoring (VP2 O16) — never do it. (VP2 O35: in-skill signoff ends at `locked`, consistent with context/tech-stack.)
