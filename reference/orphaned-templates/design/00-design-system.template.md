...
Nine tokens, symmetric around `space-md` (the baseline). The UX Designer (@ux-designer) will propose pixel values during the first design session.
...
Nine tokens, symmetric around `text-md` (body text). The UX Designer (@ux-designer) will propose sizes during the first design session.
