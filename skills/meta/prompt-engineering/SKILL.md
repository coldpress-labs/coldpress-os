...
inputs:
  graph_queries:
    - "existing skill prompts (for diff/comparison)"
  cold_file_reads:
    - "templates/prompt-snippets/*.md"
    - "templates/infrastructure/skill.md (authoring standard)"
    - "skills/utilities/advanced-elicitation/SKILL.md (elicitation trigger patterns)"
  existence_checks:
    - "templates/prompt-snippets/ (forcing-function library exists)"
outputs:
  - artifact: "Authored / revised prompt"
    location: "<skill-or-step file with embedded prompt>"
    format: "markdown"
    sacred: false
  - artifact: "Prompt rationale + version note"
    location: "_context/audit/prompt-changes-{date}.md"
    format: "markdown"
    sacred: false
---
