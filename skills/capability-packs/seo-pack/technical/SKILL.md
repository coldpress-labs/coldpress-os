...
inputs:
  graph_queries:
    - "Page nodes (page-tree for sitemap)"
    - "TechStackLock (framework-specific config)"
  cold_file_reads:
    - "_context/sacred/tech-stack.md"
    - "_context/sacred/architecture.md (NFR performance section)"
    - "coldpress.yaml (baselines.seo, baselines.performance)"
  existence_checks:
    - "Phase 3 tech-stack locked"
outputs:
  - artifact: "sitemap.xml + sitemap-index.xml"
    location: "<framework-specific path: app/sitemap.ts | public/sitemap.xml | etc.>"
    format: "xml"
    sacred: false
  - artifact: "robots.txt"
    location: "<framework-specific path: app/robots.ts | public/robots.txt>"
    format: "text"
    sacred: false
  - artifact: "Canonical + hreflang config"
    location: "<framework-specific path: middleware / per-page metadata>"
    format: "code"
    sacred: false
  - artifact: "Redirect map"
    location: "<framework-specific path: vercel.json | netlify.toml | _redirects>"
    format: "config"
    sacred: false
  - artifact: "Performance audit + remediation"
    location: "_context/audit/seo-technical-perf-v{N}.md"
    format: "markdown"
    sacred: false
---
