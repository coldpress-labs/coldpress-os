...
total_steps: 4
re_runnable: false
description: "4-step workflow: gate-check → graph-rebuild → reconciliation (conditional, from_phase=5 only) → handoff-log"
---
...
Three steps, always in sequence. Steps 1 and 3 are synchronous (Butler-driven). Step 2 (graph rebuild) is a subprocess invocation that may take 30-120 seconds on large projects.
...
| 2 | [Graph Rebuild](steps/step-02-graph-rebuild.md) | No | Warn-not-block |
...
- `.coldpress/graph/graph.json` — pre-rebuild graph (read for product-brief regen detect in Step 3)
