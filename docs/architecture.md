...
| `data/agents/` | Generated agent roster (`agent-roster.csv`); schema doc at `docs/agent-schema.md` | Framework |
...
### New Subagents
Add to `template/.claude/agents/{name}.md` following Claude Code subagent format (see `docs/agent-schema.md`). The roster (`data/agents/agent-roster.csv`) is regenerated by `build:roster`/`check:drift`.
