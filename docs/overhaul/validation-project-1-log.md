...
(works; captured for context) · **○+ positive** (a seam actively confirmed
working — the enforcement the overhaul built, verified live).
...
| O7 | Spec | ⚠ | **`config-check` is per-key, not a whole-file validator.** `coldpress config-check <key>` checks a single key (from `.coldpress/local-config.yaml`, or `coldpress.yaml` via `--file` post-D58) — there is no `coldpress` command to schema-validate the *entire* `coldpress.yaml` after a manual edit. Combined with O4 (no lite phase gate), the lite lane never machine-validates `coldpress.yaml` at all, even though the lite-spec skill instructs "record … in coldpress.yaml". | **Harvest → eval.** Candidate: a `doctor` / `config-check` whole-file mode that validates `coldpress.yaml` against its schema, so a hand-edited config is caught before Build. |
