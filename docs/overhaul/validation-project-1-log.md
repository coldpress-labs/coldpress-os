...
| O19 | Ship | ⚠ | **Lite scaffold lacks `_context/operations/`** — lite-ship's ★#2 release record location is `_context/operations/releases/REL-*.yaml`, but the lite `init` scaffold ships only `audit design handoffs implementation planning sacred testing tracking` (no `operations/`). Had to `mkdir` it to write the release record. | **Harvest → eval.** Same lite/full `_context` lane-awareness family as O2/O10/O11 — but this one bites in the *last* phase (the record has nowhere to land). Reinforces the S6b lane-aware-`_context` case: the lite lane needs `operations/` (it ships) even though it drops other full-lane dirs. |
