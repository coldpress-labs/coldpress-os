...
### WS10 remaining (recorded, not this pass)

- **B (Sonnet, mechanical):** B1 `kind`→`type` in the story-graph step files (blocking bug — a graph authored per the skill is rejected by `coldpress waves`); B2/B3 one tracking artifact + one stub path; B4 packet `owns` allowlist vs denylist; B5 one phase-handoff filename.
- **C (Sonnet):** C1 gate-runner dispatching `src/gate/checks/*` + set `phase_3_started_at`; C3 `client-timeline` from schedule.yaml; C4 trace ingests P6 + fixes the orphans-gate ordering; C7 integration-story merge skill; D1–D5 template SYSTEM.md regen + graph-verb/retired-agent purge + scaffold schema copy + doc-link fixes + dead-module deletion.
- **D (posture):** E1 doctor global-CLI + hook-wiring health + run.mjs arg forwarding; E2 `trace orphans --strict`; E3 T1+ gates the PRD security section; E4 eval-count margin + `eval_subset` mapping.
- **G (highest-leverage prevention):** `data/wiring.yaml` + `coldpress doctor --wiring` + CI — the machine-readable cross-phase artifact registry that turns this whole audit class into a permanent CI check.

**Sequencing (audit note):** A/B are prerequisites for the validation projects (a lite site would break at B1 today); do B next, then the validation projects exercise the repaired seams + satisfy the §12 runtime demos.
