...
**Remaining before the v0.4.0 tag:** the **§12 runtime demos** via the two
validation projects — the only true ship-gate blocker; all machinery is built,
wired, and CI-guarded (wiring check · structure check · staleness lint with an
empty skip-list · pack smoke · evals). **Validation Project 1 (lite lane) is
