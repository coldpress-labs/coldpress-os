...
### WS10-C remaining (larger builds, recorded)

- **C1:** a gate-runner (`coldpress gate check <phase>`) dispatching gate.json `command`s to `src/gate/checks/*`, called by phase-exit; set `phase_3_started_at` at phase entry. The 8 check functions exist + are CLI-wired; the missing piece is the engine that reads a gate.json and runs them to evaluate/record the whole gate.
- **C3:** `client-timeline` skill reading `schedule.yaml`; build the `next-task` hook or delete its references.
- **C7:** an `integration-story` skill (the IN-* sequential-merge procedure — wave-orchestration retired without a successor for this).

**Sequencing (audit note):** A/B/G are the recurrence-preventers + prerequisites; C1/C3/C7 are net-new machinery that can land before or after the validation projects.
