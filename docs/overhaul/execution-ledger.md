...
notes kept per D8) · S2.4 🟢 (D45 — seo-pack → capability-packs, matrix now 5) · S2.3/§8.14 🟢 (D46 —
templates/→authoring/; 51 orphans → reference/; governance kept, **open Q flagged**) ·
S2.5 🟢 (D47 — CHANGELOG pre-0.3 → archive). **S2 COMPLETE.**
**S3 + S3.5 + S3.6 🟢** (`overhaul/ws11-s3-deletions`, D48 — deletions + content-stale
fixes + staleness lint; httpx fixture shrink deferred) · **S5a 🟢** (D49 — links +
marketplace + REGISTRY subagents; full registry catalog rewrite = S5b, deferred) ·
**S4 🟢** (`overhaul/ws11-s4-wire-or-archive`, D50 — llm-gates wired, dev-story-output
dropped, 4 keeps documented) · **S6a 🟢** (`overhaul/ws11-s6-scaffold-diet`, D51 —
...
gate-fold). S5b (registry/tabular-doc content rewrite) is **done** (D55 — the staleness lint
now runs with an empty skip-list). One item carries past the close: **S6b**
(skills/ trim + lane-aware lifecycle — a post-v0.4.0 optimization, deferred by
design). See the open-items register (`../docs/open-items-register-2026-07-05.md`).
...
structure & diet (D38–D53) + the post-WS11 close-out (D54–D56; S5b done in D55).
**🏁 CYCLE CLOSED 2026-07-26 (D61): the §12 ship gate is CLOSED — both validation
projects complete by owner decision; the cycle ships as `0.4.0-alpha`** (alpha
retained). Every §12 box is checked with evidence or an explicit owner-accepted
disposition at the gate itself (plan §12, v3.0); three residuals carry into the
subject products' lifecycles (first actual-vs-target ops digest + `pack-harvest`
at Ancient Games; cross-project `evolve` report as run-logs accrue). Historical
position narrative follows. **Validation Project 1 (lite lane) is
COMPLETE** (`../validation/brew-bloom/`, local CLI `npm link`ed; log at
`docs/overhaul/validation-project-1-log.md`; ledger **D59**). Spec → Build →
Verify → Ship all ran end-to-end with **verdict pass** — the lite lane ships. It
earned its keep: day-one blocker **O8** (full-lane P3 gate could never pass) fixed
via valet-loop (**D58**), 6 positive seam confirmations (schema-block, tokens
pipeline, contrast validator, boundary-guard, clean-room verifier, deploy-gate),
and a harvest backlog whose headline is a **lite/full `_context` lane-awareness
cluster** (O2/O10/O11/O19) → the strongest case for the deferred **S6b** work.
**Validation Project 2 is COMPLETE — verdict PASS, closed 2026-07-26**
(`lab-originals/…/originals-p004-ancient-games`, **full lane**, IP product —
chaturanga/ancient-games collection app; log + close-out verdict at
`docs/overhaul/validation-project-2-log.md` v1.15: P1–P9 gates all earned against
a real build to a real dark production deploy, 45 findings harvested, every ✗/⚠
fixed + regression-pinned). Phase 0 done: the subject was a
...
| # | Type | Summary | Status |
|---|------|---------|--------|
| D61 | 🏁 §12 SHIP-GATE CLOSE + release bookkeeping (v0.4.0-alpha) | **The v0.4 "Enforcement" cycle is closed.** Owner decision (Aastha, 2026-07-26): VP1 + VP2 both treated as complete → validation complete; ship version stays **`0.4.0-alpha`** (no flip to 0.4.0 — pre-1.0 alpha posture retained, consistent with VP2 P0-1's no-migration-guarantee reality). **Executed:** (1) `/.claude/` root-anchored gitignore (local session transcripts were untracked noise; `template/.claude/` stays tracked). (2) **VP2 log closed** (v1.15 — phase boxes 8–11 reconciled to reality, Close-out & verdict section: PASS; P10 ops cadence + P11 retrospective/pack-harvest/framework-feedback carry to the Ancient Games product lifecycle, candidate first harvest = the O4 `interactive-app` profile). (3) **§12 gate checked off** at plan v3.0 — every box carries inline evidence or an explicit owner-accepted disposition; two accepted-as-equivalent readings (two-target deploys evidenced cross-project vercel+cloudflare-pages; the improvement loop proven by ~20 valet-loop passes with fails-before/passes-after pins); three deferrals tracked here (first actual-vs-target ops digest, `pack-harvest` run, cross-project `evolve` report — all land as the subjects operate). (4) **Public-doc refresh — the last v0.3-era surfaces retired:** `example-walkthrough.md` v5.0 (full rewrite-in-place to v0.4: lanes, real-build validation note anchoring it to VP1/VP2, graph/Graphify excised, P2/P3/P4/P5/P6/P7/P9/P10/P11 skill surfaces corrected to the shipped tree, deploy-pack staged ship path, outcome-contract thread); README (lane framing, "Shape A — v0.3.0" labels retired, stack packs corrected to 5 + seo-pack as capability pack); quick-start walkthrough pointer. (5) **CHANGELOG release cut:** `[0.4.0-alpha] — 2026-07-26` with the two-validation-run headline + a new VP2 fix-digest section (the O1–O45 fixes had never been changelogged); fresh empty `[Unreleased]`. **Green:** typecheck, **1003 tests**, lint:frontmatter, lint:staleness, check:drift, build — all clean at close. **RELEASE EXECUTED (owner-triggered):** main + tag `v0.4.0-alpha` pushed to github.com/coldpress-labs/coldpress-os 2026-07-26 (with an `npm pkg fix` bin normalization + npm-description de-stale, commit 3516110); **`@coldpress/core@0.4.0-alpha` published to npm 2026-08-09** (owner-run — 2FA-gated; dist-tag `alpha`, `latest` re-pointed per the 0.3.x convention). v0.4.0-alpha is fully shipped. | **applied (main)** |
| D60 | VP1 O19 — scaffold missing `_context/operations/` — pre-tag valet-loop fix | **The scaffold delivered no `_context/operations/` tree, yet the deploy-gate hook + 9 shipped Phase-9/10 + lite-ship skills write there.** Surfaced at VP1 Ship (the lite release record `_context/operations/releases/REL-*.yaml` had nowhere to land) but the gap is **both-lane**: `operations/{acceptance,releases,ops-digests,handover,runbooks,health-reports}` are all referenced by shipped surfaces (deploy-gate's `acceptance/`, deploy-prod/lite-ship `releases/`, client-health `health-reports/`, incident `runbooks/`, …) and none existed in `template/_context/` — so every project's own release record, client-UAT sign-off, runbooks, and observability config had no home. **Fix:** added `template/_context/operations/` + the 6 referenced subdirs, kept via `.gitkeep` like the sibling context dirs (`template/` ships in `files[]`, copied wholesale by the scaffold → both lanes get it). Regression assertion in `init-scaffold.test.ts` (a real `init` delivers `operations/releases` + `operations/acceptance`). **User-approved as the one pre-tag VP1-harvest fix** (the rest of the lite/full `_context` lane-awareness cluster stays deferred to S6b). **Green:** typecheck, **962 tests**, check:drift, lint:frontmatter, lint:staleness, build. No tags/publishes. O19 marked ✓ fixed in `validation-project-1-log.md`. | **applied (`overhaul/vp1-o19-operations-scaffold`)** |
| D59 | §12 ship gate — **Validation Project 1 (lite lane) COMPLETE** | The lite lane ran end-to-end against a real build (Brew & Bloom, a coffee-roaster micro-site at `../validation/brew-bloom/`) with the `npm link`ed local CLI acting as a published consumer: **Spec → Build → Verify → Ship, verdict pass**. Spec: ingest ledger + stack/deploy lock (`static-multipage-blog` Astro/MDX/Vercel, T0) + **walking skeleton 200 on staging** (real Astro build) + license scan (LGPL noted) + tokens/styleguide + `spec.md` (8 acceptance-tested reqs). Build: 5 stories (R1–R8), acceptance-stubs-first, **28 tests green**, `astro check` clean, `visual-verify` OK, `tokens contrast` 0 AA failures, boundary-guard enforced on a real packet. Verify: **clean-room independent verifier → pass**, adversarially cleared the tests of gaming, caught 2 real gaps the author missed. Ship: staging smoke green, **deploy-gate verified both directions**, prod human-triggered + **simulated** (never auto-deployed), release record written, uptime monitor defined. **19 findings** (full roll-up in `validation-project-1-log.md`): O8 ✗ **fixed** (D58, golden test); **6 positive seam confirmations** (O9 write-time schema block · O12 tokens pipeline · O13 contrast validator caught a real AA fail · O14 boundary-guard scope · O17 verifier independence · O18 deploy-gate); the rest ⚠ harvest — a **lite/full `_context` lane-awareness cluster** (O2/O10/O11/O19 — the lite scaffold ships full-lane subdirs yet lacks `operations/` where the ship record lands; `spec.md` unschema'd; walking-skeleton skill reads full-lane `tech-stack.md`) → the strongest consolidated case for the deferred **S6b lane-aware `_context`**; plus no lite phase gate (O4), profile/pack coupling (O5/O6/O7), design-binding reach (O15/O16). Their evals land with their fixes (a red eval pre-fix breaks CI). **VP1 = the lite half of the §12 gate; VP2 (full lane, Ancient Games) is the full half.** Bookkeeping + validation; no framework code beyond D58. No tags/publishes. | **applied (`overhaul/vp1-lite`)** |
| D58 | §12 ship gate — Validation Project 1 finding **O8** (broken full-lane P3 gate) — valet-loop fix | **First VP1 finding, caught on day one — a block-severity gate that could never pass.** The Phase-3 exit gate check `stack-pack-written-to-yaml` (`lifecycle/3-tech-stack/gate.json`) declares `path_pattern: coldpress.yaml` + `description: "coldpress.yaml stack_pack field is set"`, but its command `coldpress config-check stack_pack` read **`.coldpress/local-config.yaml`** (`configCheck` only ever read local-config) — and `stack_pack` is written to `coldpress.yaml` (stack-locking Step 4 / `coldpress-yaml-schema.md`; it isn't even in the `LocalConfig` interface). So a **correctly-locked stack reported "not found"** and the block gate could never pass — no full-lane project could exit Phase 3. Masked by `test/gate-check-config.test.ts`, which wrote `stack_pack` *into local-config.yaml* (a layout that never occurs), keeping the unit suite green while the real seam was broken. **Fix (valet-loop):** gave `config-check` an optional `--file` (`src/gate/checks/config-check.ts` now reads `coldpress.yaml` top-level keys via `yaml.parse` when `--file coldpress.yaml`, default stays `.coldpress/local-config.yaml` — back-compat; the "not found" message now names the file actually checked); threaded `--file` through `runConfigCheck` + the CLI registration; repointed the one mispointed gate command to `--file coldpress.yaml` (the other config-check use, `post_phase_3_update_ran`, is a legit local-config key — left as-is). **Golden test (fails-before / passes-after) on the REAL layout** (stack_pack in coldpress.yaml): +5 cases in `gate-check-config.test.ts`. **Verified end-to-end** against the live VP1 `brew-bloom/coldpress.yaml` (stack_pack set): the repointed gate command now `✓`, `--expected` works vs `deploy_pack: vercel`, back-compat (no `--file`) still targets local-config. **Green:** typecheck, **940 tests** (+5), check:drift, lint:frontmatter, lint:staleness, build. No tags/publishes. Logged in `validation-project-1-log.md` (O8). | **applied (`overhaul/vp1-o8-config-check-file`)** |
| D57 | Pre-validation fix pass (DV1 + DV2) | Fixes the two findings from the dynamic-verification audit (`../docs/dynamic-verification-audit-2026-07-06.md`) — the enforcement layer held under adversarial attack; both findings minor, neither a blocker. **DV1 (schema-validate under-covers WS10-era data artefacts):** the `schema-validate` PostToolUse hook only fired for sacred docs + `PATH_PATTERN_SCHEMAS` (JSON/Ajv), so the Zod-schema'd data artefacts (`outcomes.yaml` P4, `story-graph.yaml` P7, the design data files, and `HND-*.yaml` handoff packets) passed the write-time hook with "no opinion" — caught only at the phase-exit gate. Added `schemas/data-artefacts/index.ts` (a Zod path-pattern registry mirroring `schemas/design/index.ts`: outcomes/story-graph/handoff), wired `dataArtefactSchemaForPath` into `validateDocSchema` (new routing step 3b) and into `schemaApplies()` (which now also consults `designSchemaForPath` — the design registry existed in `validateDocSchema` but not in the hook's applicability check, so tokens/budgets/styleguide were uncovered too). The §4.4 "any schema'd `_context/` artefact validates on write" promise is now fully delivered. +10 adversarial tests (schemaApplies + a malformed instance denied, per type). **DV2 (boundary-guard silent fail-open):** `activePacket()` returns `undefined` both when no HND packets exist (legit) and when packets exist but none parses — so a subtly-malformed packet silently disabled the write-scope boundary with no signal. Added `unparseablePacketsPresent()` + a handler branch that emits a `context` warning ("handoff packet(s) present … but none parses … the boundary is NOT being enforced") instead of silent allow — same posture as the WS10-E1 CLI-on-PATH warning (never let enforcement vanish quietly). +1 test. **Verified end-to-end through the real `coldpress hook` stdin/stdout contract** (the audit's method): malformed `outcomes.yaml` → `decision: block` (schema `data-artefacts/outcomes.yaml`); unparseable HND → PreToolUse `additionalContext` warning. **Green:** typecheck, **935 tests** (+11), check:drift, lint:frontmatter, lint:staleness, build. | **applied (`overhaul/dv-fix-pass`)** |
| D56 | Close-out §C + §D (open-surface reconciliation) | Recorded the register's §C (resolved — close the books) and §D (parked-by-design) so the ledger's open surface matches `../docs/open-items-register-2026-07-05.md` exactly. **§C RESOLVED (verified):** C6 Conftest/rego → D53 (retired; ≥1-ADR/≥1-approver preserved as phase-gate checks). C7 `ops/security-scan` + `dep-health-check` scanner supersession → done; `skills/security/scan-*` is canonical (on disk). C8 D18 (agents+hooks into plugin) → project-local by design. C9 D6/D11 graph/PERT doc surface → subsumed by D23 (F2) + D48 (deletions + staleness lint), CI-green. C10 version split → D41 (0.4.0-alpha everywhere); marketplace agent list → D49. **§D PARKED BY DESIGN (each has a trigger, not a date):** D11 brownfield capability pack (+ 3 skills in `reference/brownfield-pending/`) — build when a real brownfield client arrives (plan §7.6). D12 remaining profiles (commerce-app/voice-agent/content-pipeline/agentic-workflow; 6/10 shipped) — harvested from the first real project of each shape via `pack-harvest`. D13 railway/digitalocean deploy packs + expo-eas stub — one `pack.yaml` each on the finished interface. D14 Agent-SDK `runTask` eval execution layer — layers on when eval volume justifies (validation projects generate the run-logs). D15 estimation-bias enrichment — wires up when validation runs produce actuals via `client-timeline`. D16 wider `context: fork` adoption — per-skill validation during real team-mode runs. D17 httpx graph-fixture shrink → **won't-do** (load-bearing for `graph.test`'s full query suite; a curated shrink is negative-value; closed accepted). **S6b** (skills/ trim + lane-aware lifecycle) = the one deferred *optimization*, post-v0.4.0 (register §5). **Net open surface = the §12 validation projects only** (the ship gate). Bookkeeping only; no code. | **applied (`overhaul/ws11-closeout`)** |
| D55 | Close-out §B4 — S5b (registry + tabular-doc content rewrite) | Executed S5b per the register §B4 recommendation (**user accepts the D49 finding — content-fix over generator**; the docs are editorial with no clean single source, and the staleness lint is the durable guard). Hand-rewrote against the current tree (8 agents · 141 plugin skills · current skill names · 5 stack packs + capability-packs · `authoring/` · 4 sacred docs): `REGISTRY.md` (rewrote the whole Phase-7/8 skill catalog — was pre-WS5: `create-epics`/`create-stories`/`parallelization-strategy`/`sprint-planning` → `story-slice`/`story-graph`/`client-timeline`/`implementation-readiness`; `wave-orchestration`/`qa-automation`/`atdd`/`test-*` → `dev-story`/`quick-dev`/`integration-story`/`verify-story`; retired-agent owners → verifier/pm/butler; `distillator`+`shard-doc` → `docs`; PERT note removed), `TEMPLATES-REGISTRY.md` (agent cells + PERT row), and the 7 tabular docs (`skill-index`, `flow-map`, `decision-trees`, `phase-subfolder-mapping`, `spec-plan-implement-review-mapping`, `subagent-phase-matrix`, `handoff-registry`) + `example-walkthrough.md` + the cross-cutting docs + `method-defaults.yaml` (per-method skill-key playbook — retired keys consolidated to current skills, verified no duplicate keys) + 3 planning-artefact schema descriptions + `pm.md` + `NOTICE.md`. **84 stale refs across ~20 files** fixed (the retired-name denylist surfaced them beyond the 10 skip-listed). **Emptied the staleness-lint `S5_REGEN_TARGETS` skip-list** + extended the DENYLIST with the retired skill *names* (`create-epics`, `create-stories`, `qa-automation`, `parallelization-strategy`, `shard-doc`, `distillator`, `sprint-planning`). **Acceptance met: `lint:staleness` GREEN with an EMPTY skip-list** — no shipped file exempt any more; the lint now guards every catalog. **Green:** typecheck, **924 tests**, check:drift, lint:frontmatter, lint:staleness, build. | **applied (`overhaul/ws11-closeout`)** |
...
| D50 | WS11-S4 (wire-or-archive — 6 design decisions) | Audit S4, one decision + rationale each. **(1) llm-gates → WIRE** (the priority — backs `verify_pack: llm-app`): the 3 normalizers (deepeval/giskard/promptfoo) convert native LLM-eval output → the `ScanResult` the security aggregator consumes, but were tested-but-unreachable (the D38 pattern). Added `coldpress llm-normalize <tool> <raw-file> --target --fail-severity --out` (`src/commands/llm-normalize.ts`) + repointed the 3 Phase-9 LLM gate skills (llm-quality-gate/prompt-regression/llm-security-scan) step-3 to invoke it instead of prose normalization. +4 tests (verified DeepEval raw → schema-valid ScanResult e2e). **(2) need-info → KEEP, no CLI** — `<NEED_INFO>` is a conversational protocol Butler parses/routes in-context; parse/route/budget are the canonical tested impl + the single source of truth for routing; a CLI verb adds an unused seam. The `dashboard/types.ts` "need-info" is a status-enum string, not a caller. **(3) handoffs/validate → KEEP as typed API** — validates by handoff id against `HANDOFF_SCHEMAS`; the `schema-validate` hook already covers path-based validation via `validateDocSchema`, so a `validate-handoff` verb would be redundant. **(4) governance/supersede → KEEP** — the supersession prompt-flow the sacred-change governance + intake/stack-eval step docs describe. **(5) decision-logger + template-builder + workflow-builder → KEEP as on-demand** — added the explicit `on-demand: true` frontmatter marker (the §S7.7 guard's intended signal for intentionally-unrouted skills). **(6) dev-story-output schema → DROP** — orphaned (no producer, no active registry route — only a stale "deferred" comment; no TS importer). Deleted `schemas/implementation/dev-story-output.schema.json` + updated the comment. Status notes added in-code to the kept modules. **Green:** typecheck, **916 tests** (+4), check:drift, lint:frontmatter, lint:staleness, build. | **applied (`overhaul/ws11-s4-wire-or-archive`)** |
| D49 | WS11-S5a (links + marketplace + REGISTRY subagents) + **scope finding** | Audit S5 (regeneration). **Done:** (1) converted the 16 cross-repo `../../../lab-hq-projects/` "Source decision:" links (404 for every consumer) to prose citations; (2) fixed root `.claude-plugin/marketplace.json` — agents `count` 11→8 + `list` to the current roster (was qa/scrum-master/communicator/valet, omitted verifier), description "11 subagents"→"8" (phase owners already current; the plugin's own marketplace manifest is generated separately by `build:skills`); (3) REGISTRY.md Subagents section → 8-agent roster + corrected the false "auto-generated via `install/generate-registry.md`" header (`install/` was deleted in D22; it's hand-maintained, guarded by `lint:staleness`). **Scope finding (deviation from the audit's premise):** `build:registries` does **not** exist — it's a placeholder comment in `check-drift.ts`, not a written-but-disabled generator. On inspection REGISTRY/TEMPLATES-REGISTRY/the "7 tabular docs" are **heavily editorial** (hand-curated counts, historical annotations, a legacy-persona "Merges" column, per-cell skill lists, rich per-handoff descriptions) AND the plugin frontmatter strips `category`/`agent`/`phases` — so there is no clean single source and a generator would *degrade* them. The durable drift-resistance the audit wanted is **already delivered by the S3 staleness lint**: once each file's content is fixed and it's removed from the lint's skip-list, the lint guards it against retired-agent/PERT/graph rot. **Remaining (focused pass):** REGISTRY's Phase-7/8 skill catalog + TEMPLATES-REGISTRY's agent-mapping predate WS4 roster surgery + WS5 skill consolidation (list deleted skills w/ retired-agent owners) — an accurate rewrite needs the current skill catalog cross-referenced; deferred rather than hand-guessed under fatigue. These files stay skip-listed until then. **User chose "focused generator" (2026-07-05)** before this finding surfaced; recommending the content-fix + un-skip path instead (lint = the guard). **Green:** typecheck, check:drift, lint:staleness. | **applied (`overhaul/ws11-s5-regen`); S5 partial** |
| D48 | WS11-S3 + S3.5 + S3.6 (deletions, content-stale fixes, staleness lint) | Audit S3/S3.5/S3.6, all pre-approved. **S3a (dead code):** deleted `src/checkpoint/`, `src/tool-signature/`, `src/orchestration/condition-reader.ts`, `src/graph/{enrich,secure-manifest}.ts`, `src/graph/render/{dot,mermaid}.ts` (kept `render/html.ts` — dashboard uses it; the audit's "render/" was over-broad), `schemas/checkpoint.schema.ts`, `schemas/handoffs/stories-to-implementation.schema.ts` (+ HANDOFF_SCHEMAS registry cleanup); deleted 5 fully-dead tests + partial-edited `handoff-schemas.test` (kept prd-to-architecture). All 8 modules verified 0-importer. Fixed 4 lifecycle docs referencing the deleted `condition-reader`. **httpx fixture shrink (S3.6-4) DEFERRED** — it's load-bearing for `graph.test`'s full query/stats/neighbors suite (specific node ids, `imports_from` edges, 144/330 counts), not just the deleted `graph-visualizer.test`; a curated shrink is a separate low-value task. **S3b (dead data/docs):** deleted `skill-catalog.csv` (+ skill-builder's catalog-maintenance refs — skills are auto-discovered), `tea-index.csv`, `data/design/stacks/*.csv`, `skills/edit/aci-primitives/*` (4, plugin 146→142), the 5 dead docs (graph-query/schema/visualizer, archetypes-guide, templates-registry) + 2 orphan docs (aci-primitives.md, checkpointer.md) + root `.DS_Store`; cleared ~15 files of resulting dangling refs. **S3.5 (content-stale):** `agents/_schema.md` rewritten to the 8-agent roster + moved to `docs/agent-schema.md` (empty `agents/` dropped from `files:`/`frameworkDirs`); `sacred-docs.md` PERT rows removed (4 sacred docs); `authoring/infrastructure/SYSTEM.md`+`skill.md` retired-agent enums → 8-agent roster; `authoring/README.md` five→four sacred-doc templates. **S3.6:** `template/.gitignore` Graphify comment fixed; 24 merged branches deleted (kept main + `wip/pre-overhaul-shape-a-propagation`). **Staleness lint (S3.5 guard):** `src/generators/staleness-lint.ts` + `lint:staleness` + CI job — retired-token denylist over shipped `.md/.yaml/.json` with a surgery-context allow-list + an S5-regen-target skip-list (REGISTRY/TEMPLATES-REGISTRY/7 tabular docs — remove from skip-list when S5 regenerates them). It surfaced 73 stragglers: fixed 3 **real schema-enum bugs** (`@qa`/`@scrum-master` in live code-review/course-correction/phase-handoff schemas), deleted the dead `graph-first-context.md`, cleared the rest. **Green:** typecheck, **912 tests** (−123 from the 5 deleted test files), check:drift, lint:frontmatter, lint:staleness, build. | **applied (`overhaul/ws11-s3-deletions`)** |
| D47 | WS11-S2.5 (CHANGELOG trim) | Audit S2: `CHANGELOG.md` (142 KB, spanning to 0.1.0) → trim pre-0.3 cycles to `CHANGELOG-archive.md`, archived out of the package. Split at the clean `---` boundary before `[0.2.0-alpha]`: moved the `[0.2.0-alpha]` + `[0.1.0-alpha]` sections (84 lines) into a new `CHANGELOG-archive.md` (with a header explaining the split); `CHANGELOG.md` keeps `[Unreleased]` → `[0.3.0-alpha]` + a pointer to the archive. `CHANGELOG-archive.md` is **not** in `package.json` `files:` nor `frameworkFiles`, so it ships/scaffolds nowhere. Modest line reduction (the bulk of the file is the current 0.3.x + Unreleased overhaul entries, which stay); satisfies the "pre-0.3 out of package" instruction. **S2 COMPLETE.** | **applied (`overhaul/ws11-s2-package-hygiene`)** |
| D46 | WS11-S2.3 (§8.14 templates prune + rename) + **two deviations** | Audit §8.14: prune orphaned `templates/design`/`contracts`/`governance` to `reference/`, then rename surviving `templates/` → `authoring/`. **Consumer-traced all 54 files** (48 design + 3 contracts + 3 governance) against the runtime surfaces (skills/lifecycle/src/scaffolded-template), distinguishing real runtime use from mere catalog listings (`TEMPLATES-REGISTRY.md`, `attribution-audit.md`, `docs/templates-registry.md`). **Findings + deviations:** (1) **all 48 design + 3 contracts have ZERO runtime consumers** — referenced only by catalogs; the v0.4 design system is the schema'd `tokens.json`/`styleguide` data files (WS10-A2), not these WDS-era markdown templates. Moved all 51 → `reference/orphaned-templates/{design,contracts}/` (audit said "45 of 48 design" — the true count is 48; moving *more* than planned, all evidence-backed). (2) **`templates/governance/` is LIVE, not "rego leftovers"** — `policies/*.rego` + `rfc-amendment.md` are referenced by the `validate-sacred-doc` skill, `docs/governance.md`, and the scaffolded `template/docs/adr`+`rfc` READMEs. Pruning it to non-shipping `reference/` would break the Conftest sacred-doc validation + the scaffolded ADR/RFC workflow, so **kept it** (→ `authoring/governance/`). **Open question flagged for the user:** the Conftest/rego governance is half-wired (its VC note says "subprocess adapter deferred pending Conftest install") — decide whether to finish wiring it or retire the feature+refs; not silently deleted here. **Rename executed:** `git mv templates → authoring` (survivors: governance, documents, infrastructure, prompt-snippets, prototype); updated `paths.ts` frameworkDirs + `package.json` `files:` (`templates`→`authoring`), `bmad.ts` imports landing path (`coldpress-os/templates/imports`→`authoring/imports`), and all runtime/test/scaffolded-doc inbound refs (`templates/<sub>`→`authoring/<sub>` across ~24 files: create-prd, prototype, architecture-design, validate-sacred-doc, docx/pdf/pptx-generator, skill-builder, prompt-*, template-builder, scaffolded ADR/RFC docs, `docs/architecture.md`, `TEMPLATES-REGISTRY`, `attribution-audit`). Left untouched: generic per-skill `templates/*.md` conventions, the local `lifecycle/4-planning/templates/` phase dir, historical VC rows, and the S3-delete `docs/templates-registry.md`. **Verified:** `npm pack` → framework-root `templates/` gone, `authoring/` ships (29 entries), orphaned `reference/` not shipped; typecheck, build, **1035 tests**, check:drift (plugin regenerated in sync) all green. `TEMPLATES-REGISTRY.md` paths repointed (full regen is S5; it still lists retired `@qa`/`@scrum-master`/`pert-chart`). | **applied (`overhaul/ws11-s2-package-hygiene`)** |
