...
empty skip-list · pack smoke · evals). **Validation Project 1 is IN PROGRESS**
(`../validation/brew-bloom/`, lite lane, local CLI `npm link`ed; log at
`docs/overhaul/validation-project-1-log.md`). It has already earned its keep —
day-one finding **O8** (the full-lane Phase-3 gate could never pass) fixed via
