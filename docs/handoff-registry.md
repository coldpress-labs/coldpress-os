...
> **Hello Butler.** Butler (the main orchestrator, see [`butler.md`](butler.md)) writes the handoff artefact at each phase exit via the `phase-transition` skill, validates high-stakes payloads on read at the next phase's entry-sync, and surfaces gate failures when validation fails.
