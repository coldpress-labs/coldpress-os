...
- **`data/design/` CSVs** — `colors.csv` (20 palettes with WCAG-AA flag + mood); `typography.csv` (12 font pairings with licensing); `styles.csv` (12 visual archetypes with palette pairings). Consumed by `brand-guidelines` (token authoring), `design-brief` (visual direction).
