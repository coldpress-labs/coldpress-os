...
tools: ["Read", "Bash"]
inputs:
  - "coldpress.yaml deploy_pack + the pack's data/deploy-packs/<name>/pack.yaml"
  - "secure/manifest.yaml + secure/.env* (secret values)"
  - "the locked stack's build config (BUILD_CMD / BUILD_DIR / NODE_VERSION)"
outputs:
  - artifact: "Staging deployment"
    location: "the pack's staging URL"
    format: "url"
---
