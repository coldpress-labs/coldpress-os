...
re_runnable: true
inputs:
  - "_input/{assets,vendor,raw,legacy,reference}/ — any material the user has already dropped in"
  - "coldpress.yaml — user/project identity"
  - ".coldpress/local-config.yaml (read-only, for resume state)"
outputs:
  - artifact: "Scaffold health + material inventory + shape classification report"
    location: "_context/tracking/intake-{date}.md"
    format: "markdown"
  - artifact: "Fully authored project context"
    location: "_context/sacred/context.md"
    format: "markdown"
    sacred: true
  - artifact: "Working-mode fields written back to coldpress.yaml"
