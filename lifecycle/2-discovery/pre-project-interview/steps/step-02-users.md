...
next_step: "step-03-constraints.md"
---

## Goal

Understand users, their needs, and the value proposition. Path diverges on `.coldpress/local-config.yaml project_shape` — brownfield asks legacy-specific questions; greenfield starts fresh.

## Instructions

### 0. Read project_shape

Read `project_shape` from `.coldpress/local-config.yaml` (set by Phase 1 intake Step 5). Values: `greenfield | brownfield | ambiguous`.
...
Continue conversational interview. Build on the vision context from Step 1. If brownfield, the prior-user questions often reveal *why this v2 exists* — surface that explicitly; it belongs in Vision (Step 1) retroactively if it reframes the story.

**Advanced-elicitation bias (per method-defaults.yaml):** if user types come back vague or generic, Butler may invoke advanced-elicitation biased toward **#4 User Persona Focus Group** and **#10 Customer Support Theater** (roleplay methods that surface real user shape).

## Output

Users section populated in the working context.md draft. Branch path (greenfield / brownfield / ambiguous) noted. `step_2_complete: true`

## Navigation

→ Proceed to [step-03-constraints.md](step-03-constraints.md)
